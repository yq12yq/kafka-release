...
public class Utils {
    ...
    /**
     * Creates a new ByteBuffer if the given buffer lower than the length requested
     * @Param existingBuffer byte buffer to check if it has enough capacity
     * @Param newLength length of the data we are trying to read
     */
    public static ByteBuffer ensureCapacity(ByteBuffer existingBuffer, int newLength) {
        if (existingBuffer == null || newLength > existingBuffer.capacity()) {
            ByteBuffer newBuffer = ByteBuffer.allocate(newLength);
            existingBuffer.flip();
            if(existingBuffer.capacity() > 0)
