...
public class NetworkReceive implements Receive {
    ...
    public NetworkReceive(int maxSize, int source) {
        this.source = source;
        this.size = ByteBuffer.allocate(4);
        this.buffer = null;
    ...
    public long readFrom(ScatteringByteChannel channel) throws IOException {
        ...
        if (size.hasRemaining()) {
            ...
            if (!size.hasRemaining()) {
                ...
                if (maxSize != UNLIMITED && requestSize > maxSize)
                    throw new IllegalStateException("Invalid request (size = " + requestSize + " larger than " + maxSize + ")");
